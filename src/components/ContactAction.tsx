    ...
    <>
      ...
      <div className="col-span-full mt-[5rem] flex w-full items-center justify-between">
          ...
          {linkOrder.map((link) => (
            <span
              ...
              className={`h-[0.5rem] w-[0.5rem] rounded-full ${
                pathname === link ? "bg-black-100" : "bg-black-10"
              }`}
            />
          ))}
        </div>
        <div className="flex gap-[1rem]">
          ...
          <button className="btn-blue" onClick={() => nextButtonHandle()}>
            {currLinkIndex === linkOrder.length - 1 ? "Submit" : "Next"}
          </button>
        </div>
      </div>
      {currLinkIndex === linkOrder.length - 1 && (
        <p className="text-sm mt-[1.5rem] text-black-70">
          By clicking Submit, you agree to our Privacy Policy, and allow
          Designers Clan to use this information for marketing purposes.
        </p>
      )}
    </>
  );
